...
export const createGame = async (req: Request, res: Response) => {
  try {
    ...
    const newGame = await prisma.casinoGame.create({
      data: {
        title: data.title,
        slug,
...
export const updateGame = async (req: Request, res: Response) => {
  try {
    ...
    const updatedGame = await prisma.casinoGame.update({
      where: { id },
      data: {
...
export const deleteGame = async (req: Request, res: Response) => {
  try {
    const id = String(req.params.id);
    await prisma.casinoGame.delete({
      where: { id },
    });
...
export const getRecommendedGames = async (req: Request, res: Response) => {
  try {
    const games = await prisma.casinoGame.findMany({
      where: {
        OR: [
