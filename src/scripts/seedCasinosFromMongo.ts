...
function cleanWithdrawalTime(raw?: string | null): string | null {
  if (!raw) return null;
...
interface MongoCasinoDoc {
  ...
  responsibleGamblingDescription?: string;
  visits?: number;
  games?: {
    slots?: boolean;
    liveCasino?: boolean;
...
async function loadCasinoData(): Promise<MongoCasinoDoc[]> {
  const possiblePaths = [
...
async function main() {
  ...
  const countryMap = new Map<string, string>(); // lowercase name -> id
  ...
  for (let i = 0; i < casinosData.length; i++) {
    ...
    try {
      ...
      if (doc.generalInfo?.established) {
        ...
        if (match) establishedYear = parseInt(match[0], 10);
      }
      ...
      if (doc.paymentInfo?.minimumDeposit) {
        ...
        if (match) minDeposit = parseFloat(match[0]);
      }

      // Support methods
      const supportMethods: string[] = [];
      const supportDesc = doc.customerSupportDescription || '';
      if (/live\s*chat/i.test(supportDesc)) supportMethods.push('Live Chat 24/7');
      if (/email/i.test(supportDesc)) supportMethods.push('Email Support');
      if (/telegram/i.test(supportDesc)) supportMethods.push('Telegram Support');
      if (/phone/i.test(supportDesc)) supportMethods.push('Phone Support');
      ...
      if (tagsList.length > 0) {
        ...
        const tagMappings: { casino_id: string; tag_id: string }[] = [];
      ...
      if (Array.isArray(doc.availableCountries) && doc.availableCountries.length > 0) {
        ...
        const countryMappings: { casino_id: string; country_id: string }[] = [];
      ...
      const categoryMappings: { casino_id: string; category_id: string }[] = [];

      // Link to online-casino category by default
      const onlineCatId = categoryMap.get('online-casino');
      if (onlineCatId) categoryMappings.push({ casino_id: casinoId, category_id: onlineCatId });
