...
async function enrichCasinoData() {
  ...
  const ybets = await prisma.casino.findFirst({
  ...
  await prisma.casinoBonus.deleteMany({
    where: { casino_id: ybets.id },
  });

  await prisma.casinoBonus.createMany({
    data: [
      {
        casino_id: ybets.id,
  ...
  const existingPros = await prisma.casinoPros.findMany({ where: { casino_id: ybets.id } });
  if (existingPros.length === 0) {
    await prisma.casinoPros.createMany({
      data: [
  ...
  const existingCons = await prisma.casinoCons.findMany({ where: { casino_id: ybets.id } });
  if (existingCons.length === 0) {
    await prisma.casinoCons.createMany({
      data: [
  ...
  const otherCasinos = await prisma.casino.findMany({
    where: {
      id: { not: ybets.id },
      gaming_tools: { equals: [] },
    },
