    ...
    try {
        ...
        const newCasino = await prisma_1.prisma.casino.create({
            data: {
                ...
                card_badge: casinoData.card_badge ? truncateString(casinoData.card_badge, 100) : null,
                ...
                game_types: game_types ? {
                    create: game_types.map((gtId) => ({ game_type_id: gtId }))
                } : undefined,
                available_countries: available_countries ? {
                    create: available_countries.map((countryId) => ({ country_id: countryId }))
                } : undefined,
                restricted_countries: restricted_countries ? {
                    create: restricted_countries.map((countryId) => ({ country_id: countryId }))
                } : undefined,
                ...
                badges: badges ? {
                    create: badges.map((badgeId) => ({ badge_id: badgeId }))
                } : undefined,
...
exports.createCasino = createCasino;
const updateCasino = async (req, res) => {
    try {
        const id = String(req.params.id);
        const { languages, bonuses, features, pros, cons, payment_methods, currencies, game_providers, tags, categories, game_types, available_countries, restricted_countries, screenshots, gallery_videos, faqs, badges, reviews_summary, ...casinoData } = req.body;
        const updatedCasino = await prisma_1.prisma.$transaction(async (tx) => {
            // Clear out existing relation entries
            if (languages !== undefined)
                await tx.casinoLanguage.deleteMany({ where: { casino_id: id } });
            if (bonuses !== undefined)
                await tx.casinoBonus.deleteMany({ where: { casino_id: id } });
            if (features !== undefined)
                await tx.casinoFeature.deleteMany({ where: { casino_id: id } });
            if (pros !== undefined)
                await tx.casinoPros.deleteMany({ where: { casino_id: id } });
            if (cons !== undefined)
                await tx.casinoCons.deleteMany({ where: { casino_id: id } });
            if (payment_methods !== undefined)
                await tx.casinoPaymentMethod.deleteMany({ where: { casino_id: id } });
            if (currencies !== undefined)
                await tx.casinoCurrency.deleteMany({ where: { casino_id: id } });
            if (game_providers !== undefined)
                await tx.casinoGameProvider.deleteMany({ where: { casino_id: id } });
            if (tags !== undefined)
                await tx.casinoTagMapping.deleteMany({ where: { casino_id: id } });
            if (categories !== undefined)
                await tx.casinoCategoryMapping.deleteMany({ where: { casino_id: id } });
            if (game_types !== undefined)
                await tx.casinoGameTypeMapping.deleteMany({ where: { casino_id: id } });
            if (available_countries !== undefined)
                await tx.casinoAvailableCountry.deleteMany({ where: { casino_id: id } });
            if (restricted_countries !== undefined)
                await tx.casinoRestrictedCountry.deleteMany({ where: { casino_id: id } });
            if (screenshots !== undefined)
                await tx.casinoScreenshot.deleteMany({ where: { casino_id: id } });
            if (gallery_videos !== undefined)
                await tx.casinoGalleryVideo.deleteMany({ where: { casino_id: id } });
            if (faqs !== undefined)
                await tx.casinoFaq.deleteMany({ where: { casino_id: id } });
            if (badges !== undefined)
                await tx.casinoBadgeMapping.deleteMany({ where: { casino_id: id } });
            if (reviews_summary !== undefined)
                await tx.casinoReviewsSummary.deleteMany({ where: { casino_id: id } });
            return tx.casino.update({
                ...
                data: {
                    ...
                    card_badge: casinoData.card_badge !== undefined ? (casinoData.card_badge ? truncateString(casinoData.card_badge, 100) : null) : undefined,
...
exports.updateCasino = updateCasino;
const deleteCasino = async (req, res) => {
    try {
        const id = String(req.params.id);
        await prisma_1.prisma.casino.delete({
            where: { id }
        });
...
exports.deleteCasino = deleteCasino;
const exportCasinos = async (req, res) => {
    try {
        const casinos = await prisma_1.prisma.casino.findMany({
        ...
        const worksheetData = casinos.map(casino => ({
            ...
            'Support Methods': Array.isArray(casino.support_methods) ? casino.support_methods.join(', ') : '',
            'Meta Keywords': Array.isArray(casino.meta_keywords) ? casino.meta_keywords.join(', ') : '',
...
exports.exportCasinos = exportCasinos;
const exportCasinosTemplate = async (req, res) => {
    try {
...
exports.exportCasinosTemplate = exportCasinosTemplate;
const parseSplitField = (val) => {
    if (!val || typeof val !== 'string')
        return [];
    return val.split(/[,\n]/).map((s) => s.trim()).filter(Boolean);
};
const parseBonuses = (val) => {
    if (!val || typeof val !== 'string')
        return [];
    return val.split(';').map(entry => {
        const parts = entry.trim().split('|');
        const bonus = { title: parts[0]?.trim() || '', type: parts[1]?.trim() || 'deposit', amount: parts[2]?.trim() || '' };
        for (let i = 3; i < parts.length; i++) {
            const p = parts[i].trim();
            if (p.startsWith('code:'))
                bonus.bonus_code = p.replace('code:', '').trim();
            else if (p.startsWith('wagering:'))
                bonus.wagering_requirement = p.replace('wagering:', '').trim();
        }
        return bonus;
    }).filter(b => b.title);
};
...
const importCasinos = async (req, res) => {
    try {
        if (!req.file) {
        ...
        for (const row of data) {
            try {
                ...
                let existingId = null;
                if (rowId) {
                    const byId = await prisma_1.prisma.casino.findUnique({ where: { id: rowId } });
                    if (byId)
                        existingId = byId.id;
                }
                if (!existingId && slug) {
                    const bySlug = await prisma_1.prisma.casino.findUnique({ where: { slug } });
                    if (bySlug)
                ...
                if (existingId) {
                    await prisma_1.prisma.$transaction(async (tx) => {
                        if (languages.length)
                            await tx.casinoLanguage.deleteMany({ where: { casino_id: existingId } });
                        if (bonuses.length)
                            await tx.casinoBonus.deleteMany({ where: { casino_id: existingId } });
                        if (features.length)
                            await tx.casinoFeature.deleteMany({ where: { casino_id: existingId } });
                        if (pros.length)
                            await tx.casinoPros.deleteMany({ where: { casino_id: existingId } });
                        if (cons.length)
                            await tx.casinoCons.deleteMany({ where: { casino_id: existingId } });
                        if (payment_methods.length)
                            await tx.casinoPaymentMethod.deleteMany({ where: { casino_id: existingId } });
                        if (currencies.length)
                            await tx.casinoCurrency.deleteMany({ where: { casino_id: existingId } });
                        if (game_providers.length)
                            await tx.casinoGameProvider.deleteMany({ where: { casino_id: existingId } });
                        if (faqs.length)
                            await tx.casinoFaq.deleteMany({ where: { casino_id: existingId } });
                    ...
                    await prisma_1.prisma.casino.create({
                        data: {
                            ...casinoData,
                            languages: languages.length ? { create: languages.map(l => ({ language: l })) } : undefined,
                            bonuses: bonuses.length ? { create: bonuses.map(b => ({ title: b.title, type: b.type, amount: b.amount, bonus_code: b.bonus_code || null, wagering_requirement: b.wagering_requirement || null, sort_order: 0 })) } : undefined,
                ...
                results.failed++;
                results.errors.push(`Failed to import row "${row['Name'] || 'Unknown'}": ${error.message}`);
...
exports.importCasinos = importCasinos;
const updateCasinoRanking = async (req, res) => {
    try {
...
exports.updateCasinoRanking = updateCasinoRanking;
const updateCasinoPosition = async (req, res) => {
    try {
        const id = String(req.params.id);
        ...
        const casino = await prisma_1.prisma.casino.update({
            where: { id },
