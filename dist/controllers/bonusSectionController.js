...
exports.deleteBonusSection = exports.updateBonusSection = exports.createBonusSection = exports.getBonusSectionById = exports.getBonusSectionBySlug = exports.getBonusSections = void 0;
...
exports.getBonusSections = getBonusSections;
const getBonusSectionBySlug = async (req, res) => {
    try {
        const { slug } = req.params;
        const section = await prisma_1.prisma.bonusSection.findUnique({
            where: { slug },
            include: {
                items: {
                    orderBy: { sort_order: 'asc' },
...
exports.getBonusSectionBySlug = getBonusSectionBySlug;
const getBonusSectionById = async (req, res) => {
    try {
        const { id } = req.params;
        const section = await prisma_1.prisma.bonusSection.findUnique({
            where: { id },
...
const createBonusSection = async (req, res) => {
    try {
        ...
        // Ensure unique slug
        let existing = await prisma_1.prisma.bonusSection.findUnique({ where: { slug } });
        let counter = 1;
        const baseSlug = slug;
        while (existing) {
            slug = `${baseSlug}-${counter}`;
            existing = await prisma_1.prisma.bonusSection.findUnique({ where: { slug } });
            counter++;
        }
        const safeSort = sort_order !== undefined && !isNaN(parseInt(String(sort_order), 10))
        ...
        if (Array.isArray(items) && items.length > 0) {
            for (let i = 0; i < items.length; i++) {
                const item = items[i];
                if (item.casino_id) {
                    await prisma_1.prisma.bonusSectionItem.create({
                        data: {
        ...
        const createdWithItems = await prisma_1.prisma.bonusSection.findUnique({
            where: { id: section.id },
            include: {
                items: {
                    orderBy: { sort_order: 'asc' },
...
exports.createBonusSection = createBonusSection;
const updateBonusSection = async (req, res) => {
    try {
        ...
        if (badge_text !== undefined)
            updateData.badge_text = badge_text ? badge_text.trim() : null;
        ...
        await prisma_1.prisma.bonusSection.update({
            where: { id },
            data: updateData,
        });
        ...
        if (Array.isArray(items)) {
            // Delete old items
            await prisma_1.prisma.bonusSectionItem.deleteMany({
                where: { section_id: id },
            });
            // Insert new items
            if (items.length > 0) {
                const validItems = items
                    .filter((item) => item.casino_id)
                    .map((item, idx) => ({
                    section_id: id,
                    casino_id: item.casino_id,
                ...
                if (validItems.length > 0) {
                    await prisma_1.prisma.bonusSectionItem.createMany({
                        data: validItems,
                    });
                }
            }
        }
        const updated = await prisma_1.prisma.bonusSection.findUnique({
            where: { id },
            include: {
                items: {
                    orderBy: { sort_order: 'asc' },
...
exports.updateBonusSection = updateBonusSection;
const deleteBonusSection = async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.prisma.bonusSection.delete({
            where: { id },
        });
...
exports.deleteBonusSection = deleteBonusSection;
//# sourceMappingURL=bonusSectionController.js.map