...
exports.deleteGuide = exports.updateGuide = exports.createGuide = exports.getGuideById = exports.getGuideBySlug = exports.getGuideCategories = exports.getGuides = void 0;
...
const getGuideCategories = async (req, res) => {
    try {
...
const getGuideBySlug = async (req, res) => {
    try {
        ...
        const relatedGuides = await prisma_1.prisma.guide.findMany({
            where: {
...
const createGuide = async (req, res) => {
    try {
        const { title, slug: customSlug, category, excerpt, content, featured_image, meta_title, meta_description, status, published_at, author_name, sort_order, } = req.body;
        if (!title || !title.trim()) {
        ...
        // Ensure unique slug
        let existing = await prisma_1.prisma.guide.findUnique({ where: { slug } });
        let counter = 1;
        const baseSlug = slug;
        while (existing) {
            slug = `${baseSlug}-${counter}`;
            existing = await prisma_1.prisma.guide.findUnique({ where: { slug } });
            counter++;
        }
        const safeDate = published_at && !isNaN(new Date(published_at).getTime())
...
const updateGuide = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, slug, category, excerpt, content, featured_image, meta_title, meta_description, status, published_at, author_name, sort_order, } = req.body;
        const data = {};
        if (title !== undefined)
        ...
        if (excerpt !== undefined)
            data.excerpt = excerpt ? excerpt.trim() : null;
        if (content !== undefined)
            data.content = content ? content.trim() : null;
        if (featured_image !== undefined)
            data.featured_image = featured_image ? featured_image.trim() : null;
        if (meta_title !== undefined)
            data.meta_title = meta_title ? meta_title.trim() : null;
        if (meta_description !== undefined)
            data.meta_description = meta_description ? meta_description.trim() : null;
        if (status !== undefined)
            data.status = status;
        ...
        if (author_name !== undefined)
            data.author_name = author_name ? author_name.trim() : 'Casino Expert';
...
const deleteGuide = async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.prisma.guide.delete({
            where: { id },
        });
