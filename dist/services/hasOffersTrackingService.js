...
exports.HasOffersTrackingService = void 0;
const prisma_1 = require("../prisma");
/**
 * Service for HasOffers tracking and URL generation
 */
class HasOffersTrackingService {
    /**
     ...
     */
    static async generateAffiliateUrl(click_id, casino_id) {
        const click = await prisma_1.prisma.hasOffersClick.findUnique({
            where: { click_id },
            include: { casino: true }
        });
        if (!click) {
     ...
     */
    static generatePostbackUrl(baseUrl, casino_id) {
     ...
     */
    static async getTrackingParameters(click_id) {
        const click = await prisma_1.prisma.hasOffersClick.findUnique({
            where: { click_id },
            include: {
                casino: {
    ...
    static async recordConversion(params) {
        const { transaction_id, offer_id, affiliate_id, aff_sub, aff_sub2, aff_sub3, aff_sub4, aff_sub5, conversion_type = 'lead', payout, sale_amount, status = 'pending', ip_address, user_agent, country, referring_url } = params;
        ...
        if (existingConversion) {
            ...
            return await prisma_1.prisma.hasOffersConversion.update({
                where: { transaction_id },
                data: {
                    status,
     ...
     */
    static async getCasinoStats(casino_id, dateRange) {
        const where = { casino_id };
        if (dateRange) {
        ...
        const [totalConversions, totalRevenue, totalPayout, statusCounts, typeCounts, recentConversions] = await Promise.all([
            prisma_1.prisma.hasOffersConversion.count({ where }),
            prisma_1.prisma.hasOffersConversion.aggregate({
                where,
            ...
            prisma_1.prisma.hasOffersConversion.groupBy({
                by: ['status'],
                where,
                _count: true
            }),
            ...
            prisma_1.prisma.hasOffersConversion.findMany({
                where,
                take: 10,
...
exports.HasOffersTrackingService = HasOffersTrackingService;
//# sourceMappingURL=hasOffersTrackingService.js.map